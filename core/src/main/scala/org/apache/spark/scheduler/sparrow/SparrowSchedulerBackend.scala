...
class SparrowSchedulerBackend(scheduler: SparrowScheduler)
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      case RegisterExecutor(executorId, executorRef, cores, logUrls) =>
        ...
        if (executorDataMap.contains(executorId)) {
          ...
          val data = new ExecutorData(executorRef, executorAddress, executorAddress.host,
            cores, cores, logUrls)
  ...
  /**
    ...
    *
    * @param numExecutors         The total number of executors we'd like to have. The cluster
    *                             manager shouldn't kill any running executor to reach this number,
    *                             but, if all existing executors were to die, this is the number of
    *                             executors we'd want to be allocated.
