 ...
 * {{{
 * TERADATA_DOCKER_IMAGE_NAME=mysql:5.7.31
 *         ./build/sbt -Pdocker-integration-tests "testOnly *v2*TeradataIntegrationSuite"
 *
 * }}}
 *
 */
@DockerTest
class TeradataIntegrationSuite extends DockerJDBCIntegrationSuite with V2JDBCTest {
  override val catalogName: String = "teradata"
  override val db = new DatabaseOnDocker {
    override val imageName = sys.env.getOrElse("TERADATA_DOCKER_IMAGE_NAME", "mysql:5.7.31")
